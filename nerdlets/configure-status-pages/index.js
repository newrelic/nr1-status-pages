...
export default class ConfigureStatusPages extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      loading: true,
      keyObject: {
        key: entityGuid ? entityGuid : accountId,
        type: entityGuid ? 'entity' : 'account',
      },
    };
  ...
  async componentDidMount() {
    ...
    if (
      ...
      for (var i = 0; i < chuckedAccounts.length; i++) {
        let queryString = chuckedAccounts[i]
          .map(
            (account, index) =>
              ` ${'a'.repeat(index + 1)}: account(id: ${
                account.id
              }) { nerdStorage { document(collection: "${HOST_NAMES_COLLECTION_KEY}", documentId: "${HOST_NAMES_DOCUMENT_ID}") } } `
          )
          .join(' ');
  ...
  async checkAddToDashBoard(hostNameObject) {
    ...
    if (hostNameObject.isSelected) {
      ...
      hostNames.splice(
        hostNames.findIndex(val => val.hostName === hostNameObject.hostName),
        1
      );
    }
    try {
  ...
  searchForDependencyTags(relationship) {
    return this.state.hostNames.find(
      hostNameObject =>
        hostNameObject.tags &&
        hostNameObject.tags.includes(relationship.name.toLowerCase())
    );
  }
  ...
  async getEntitityRelationShips() {
    ...
    try {
      ...
      const relationships =
        relationshipsResults.data.actor.entity.relationships;
      const external_relationships = relationships
        .filter(
          relationship =>
            relationship.target.entity.entityType !== 'APM_APPLICATION_ENTITY'
        )
        .map(filteredResults => {
          return filteredResults.target.entity.name;
        });
      const distinct_external_rel = [...new Set(external_relationships)];
      this.setState({
        relationships: distinct_external_rel.map(rel => {
          return { name: rel, isSelected: false };
        }),
      });
    } catch (err) {
      console.log(err);
        ...
        <li key={relationship.name} className="modal-list-item">
          ...
          <div className="button-bar">
            ...
            {!foundMatch && (
              <Button
                className={`btn-white ${
                  relationship.isSelected ? 'selected' : ''
                }`}
  ...
  generateStatusPages(hostNames) {
    return hostNames.map(hostname => {
      hostname.isSelected = !!this.state.hostNames.find(
        val => val.hostName === hostname.hostName
      );
      return (
        <GridItem
        ...
        >
          <div
            onClick={this.checkAddToDashBoard.bind(this, hostname)}
    ...
    return (
      <div>
        ...
        <Tabs>
          ...
          {/* TODO!: Disable this until we can come back to it */}
          {/* {entityGuid && (
